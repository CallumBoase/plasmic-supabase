...
const buildSupabaseQueryWithDynamicFilters = ({
  ...
  | PostgrestFilterBuilder<any, any, null> => {
  ...
  if (operation === "select") {
    if (!columns)
      throw new Error(
        "Error in buildSupabaseQueryWithDynamicFilters: columns must be a string like '*' or 'id, name' for select operation."
      );
    if (returnCount === "none" || !returnCount) {
      ...
      supabaseQuery = supabase
        .from(tableName)
        .select(columns, { count: returnCount });
    }
  } else if (operation === "insert") {
    if (!dataForSupabase)
      throw new Error(
        "Error in buildSupabaseQueryWithDynamicFilters: dataForSupabase must be an object with key-value pairs for insert operation."
      );
    supabaseQuery = supabase.from(tableName).insert(dataForSupabase);
  } else if (operation === "update") {
    if (!dataForSupabase)
      throw new Error(
        "Error in buildSupabaseQueryWithDynamicFilters: dataForSupabase must be an object with key-value pairs for update operation."
      );
    supabaseQuery = supabase.from(tableName).update(dataForSupabase);
  } else if (operation === "upsert") {
    if (!dataForSupabase)
      throw new Error(
        "Error in buildSupabaseQueryWithDynamicFilters: dataForSupabase must be an object with key-value pairs for upsert operation."
      );
    ...
    throw new Error(
      "Error in buildSupabaseQueryWithDynamicFilters: Invalid operation. Must be select, insert, update, upsert, or delete."
    );
  }
  ...
  if (supabaseQuery) {
    ...
    if (operation === "select" && orderBy && orderBy.length > 0) {
      orderBy.forEach((orderField) => {
        //Typescript ignore next line because it doesn't like the dynamic operator and no benefit for enforcing safety
        //@ts-ignore-next-line
        supabaseQuery = supabaseQuery.order(orderField.fieldName, {
          ascending: orderField.direction === "asc",
        });
      });
    }
    ...
    if (operation === "select") {
      if (limit && offset) {
        ...
        throw new Error(
          "Invalid combination of Supabase API operations. Offset operation not allowed without limit."
        );
      }
    }
  } else {
  ...
  // For mutations, add select() after mutate if runSelectAfterMutate is true
  if(
    supabaseQuery &&
    (operation === "insert" || 
    operation === "update" || 
    operation === "upsert" || 
    operation === "delete") &&
    runSelectAfterMutate === true
  ) {
    supabaseQuery = supabaseQuery.select(columns || undefined);
  }
